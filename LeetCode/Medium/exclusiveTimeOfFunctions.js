...
const exclusiveTime = (n, logs) => {
  ...
  for (let i = 0; i < logs.length; i++) {
    ...
    splitLogs[i][0] = parseInt(splitLogs[i][0]);
    splitLogs[i][2] = parseInt(splitLogs[i][2]);
  }

  if (n === 1)
    return [splitLogs[splitLogs.length - 1][2] - splitLogs[0][2] + 1];

  let end = splitLogs[0][2];

  for (let i = 0; i < splitLogs.length; i++) {
    let id = splitLogs[i][0];
    let status = splitLogs[i][1];
    let timestamp = splitLogs[i][2];

    let prevStatus;

    if (i > 0) prevStatus = splitLogs[i - 1][1];

    if (status === "start") {
      if (prevStatus === "start") {
        time[splitLogs[i - 1][0]] += timestamp - end;
        end = timestamp;
      }
    } else {
      time[id] += timestamp - end + 1;
      end = timestamp + 1;
      splitLogs.splice(i - 1, 2);
      i -= 2;
    }
  }
